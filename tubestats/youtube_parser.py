...
def channel_parser(
    youtube: googleapiclient.discovery.Resource, for_parse: str
) -> Sequence[str]:
    """
    ...
    if len(for_parse) == 11:
        ...
        video_id = m.group(4)  # type: ignore
        if video_id == "channel":
            return m.group(6)  # type: ignore
        elif video_id == "user":
            channel_username = m.group(6)  # type: ignore
